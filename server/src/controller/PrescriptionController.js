  ...
  async createPrescription(req, res) {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Thiếu dữ liệu hoặc danh sách thuốc rỗng" });
      }
      if (existingTestOrder) {
        return res.status(400).json({
          message:
            "Bệnh nhân đang chờ kết quả xét nghiệm. Không thể nhập chuẩn đoán.",
        });
      }
      ...
      await sendNotificationToUser(updatedAppointment.user_id, {
        title: "Kết quả khám & đơn thuốc",
        body: "Đơn thuốc và kết quả khám của bạn đã được tạo.",
  ...
  async getPrescriptionByAppoinmentId(req, res) {
    ...
    try {
      ...
      if (!prescription) {
        return res
          .status(404)
          .json({ message: "Không tìm thấy đơn thuốc cho cuộc hẹn này." });
      }
      ...
      res.status(200).json({
        ...
        medicine: prescription.medicines.map(
          (med) => `${med.name} - ${med.quantity} ${med.unit} - ${med.dosage}`
        ),
