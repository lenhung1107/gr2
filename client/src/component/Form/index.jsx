...
const Form = ({ onClose, service, date, time, appointmentType }) => {
  ...
  useEffect(() => {
    const fetchSavedPatients = async () => {
      ...
      if (user && isForSelf === false) {
        try {
          const res = await fetch(`http://localhost:4000/appointment/getAppoinmentByUserId/${user._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:4000/appointment/bookAppointment", {
        method: "POST",
